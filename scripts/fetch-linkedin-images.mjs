/**
 * Extracts og:image + JSON-LD gallery images from LinkedIn post URLs.
 * Run: node scripts/fetch-linkedin-images.mjs
 */
...
async function fetchImages(url) {
  ...
  const cover = og ? decodeUrl(og[1]) : null;

  const gallery = [];
  const re = /"url":"(https:\\\/\\\/media\.licdn\.com\\\/dms\\\/image\\\/[^"]+feedshare[^"]+)"/g;
  let m;
  while ((m = re.exec(html)) !== null) {
    const u = decodeUrl(m[1].replace(/\\\//g, '/'));
    if (!gallery.includes(u)) gallery.push(u);
  }

  if (gallery.length === 0 && cover) gallery.push(cover);

  return { cover: cover ?? gallery[0] ?? null, gallery };
}

const out = {};
for (const p of posts) {
  try {
    const data = await fetchImages(p.url);
    out[p.id] = { sourceUrl: p.url, ...data };
    console.error(`OK ${p.id}: cover + ${data.gallery.length} gallery`);
  } catch (e) {
